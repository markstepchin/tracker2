  ...
  return (
    <div className="max-w-sm mx-auto">
      <div className="flex justify-between p-4 bg-white rounded-md shadow my-10">
        ...
        <a href="/in-progress">
          <button
            class="bg-gray-800 text-white font-bold rounded-full px-4 py-2 shadow-lg uppercase "
          >
            ...
            {Object.keys(workout).map(exercise => {
              ...
              return (
                <div className="m-0 py-1">
                  <h5>{exercise}</h5>
                  <div className="text-sm text-left text-gray-500">{sets.map((set, i) => <span>{i !== 0 && ', '}{set.weight}lbs x {set.reps}</span>)}</div>
                </div>)
            })}
          </div>
